...
func (st DBstore) validateAlertRule(alertRule ngmodels.AlertRule) error {
	...
	for key := range alertRule.Labels {
		if strings.Contains(key, " ") {
			return fmt.Errorf("%w: invalid label: %s; label keys should not contain spaces", ngmodels.ErrAlertRuleFailedValidation, key)
		}
	}

	for key := range alertRule.Annotations {
		if strings.Contains(key, " ") {
			return fmt.Errorf("%w: invalid annotation: %s; annotation keys should not contain spaces", ngmodels.ErrAlertRuleFailedValidation, key)
		}
	}
